...
#EXTRA WORK
#For sorting according to region and the customers and their max revenues.

sales %>% group_by(region, custname) %>% summarise(Maxrev= max(revenue)) %>% arrange(region,desc(custname)) %>% top_n(n=2)
